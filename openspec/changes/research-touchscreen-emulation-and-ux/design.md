...
- The panel is confirmed (2026-07-22): the official Raspberry Pi Touch Display 2, 5-inch — 720x1280 native portrait, DSI, 5-finger capacitive, viewing area 63x111.5 mm, GPIO-powered, rotatable. Its density is ~11.4 px/mm (~290 ppi), roughly phone-class, which is far denser than the common third-party 800x480 5-inch DSI panels (~192 ppi). The design target is therefore a 720x1280 portrait capacitive touchscreen, and touch-target/type sizes must be anchored in physical mm and converted at ~11.4 px/mm (a generic "48 px" minimum is ~4.2 mm here and too small).
...
The research will produce an initial UI/UX direction constrained to 720x1280 portrait, physically-sized (mm-anchored) touch targets, high contrast for low light, glanceable status, and a clear division of roles between the touchscreen, CV-driven selection, and the Bluetooth control channel.

Rationale: the device is operated live, often in the dark, by finger; legibility, target size, and immediate feedback dominate, and the touchscreen is primarily status/config/manual-override while CV drives real-time selection. Because the confirmed panel is ~290 ppi, target and type sizes are defined in mm and converted to px, not lifted from generic px minimums.
...
- The 720x1280 portrait surface is a tall, narrow strip that may not fit the intended information -> Mitigation: prioritize glanceable status and large mm-sized targets in a vertical stack; move detail to secondary screens or the phone/Bluetooth surface.
...
- Panel confirmed (2026-07-22): official Raspberry Pi Touch Display 2, 5-inch, 720x1280 portrait DSI, 5-finger capacitive. Remaining: is it mounted native portrait or rotated to landscape at the rack, and what touch-input transform does that require?
